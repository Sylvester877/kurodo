// Fill in the v0.3.39 release notes + push the version-bump commit.
...
const HDR = { Authorization: `Bearer ${token}`, 'User-Agent': 'kurodo-release-script', Accept: 'application/vnd.github+json' }
...
if (!list.id) { console.error('release not found:', JSON.stringify(list).slice(0, 200)); process.exit(1) }
console.log('release id:', list.id, '| current body empty:', !list.body)
