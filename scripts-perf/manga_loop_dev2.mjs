import puppeteer from 'puppeteer'
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'
const OUT = path.join(path.dirname(path.dirname(fileURLToPath(import.meta.url))), 'screenshots')
const CHROME = process.env.CHROME_PATH || 'C:\Program Files\Google\Chrome\Application\chrome.exe'
const chromePath = fs.existsSync(CHROME) ? CHROME : undefined
const BASE = 'http://127.0.0.1:5173'
const b = await puppeteer.launch({ headless:'new', executablePath: chromePath, args:['--no-sandbox','--disable-gpu','--window-size=1440,900'] })
const p = await b.newPage()
await p.setViewport({width:1440,height:900})
await p.goto(`${BASE}/`, {waitUntil:'domcontentloaded', timeout:25000})
await new Promise(r=>setTimeout(r,3000))
await p.evaluate(()=> { const b=[...document.querySelectorAll('button')].find(x=> x.innerText.includes('Skip setup')); if(b) b.click() })
await new Promise(r=>setTimeout(r,2500))
// Check API via browser fetch (same origin as Vite)
const api = await p.evaluate(async ()=> {
  const r = await fetch('/api/atsu/pages/sVC2A/yaCvs')
  const t = await r.text()
  return { status: r.status, snippet: t.slice(0,600) }
})
console.log('[dev api]', api.status, api.snippet.slice(0,500))
await p.goto(`${BASE}/manga/read/yaCvs?manga=sVC2A&source=atsu&malId=13`, {waitUntil:'domcontentloaded', timeout:25000})
await new Promise(r=>setTimeout(r,8000))
const diag = await p.evaluate(()=> {
  const text = document.body.innerText.slice(0,1000)
  const hasPill = [...document.querySelectorAll('button')].some(b=> b.innerText.includes('Ch.'))
  const spine = !!document.querySelector('[role="progressbar"]')
  const pillText = [...document.querySelectorAll('button')].find(b=> b.innerText.includes('Ch.'))?.innerText.slice(0,80) || null
  return { text: text.slice(0,600), hasPill, spine, pillText }
})
console.log(JSON.stringify(diag,null,2))
await p.screenshot({path: path.join(OUT, 'loop-02-dev-after-proxy.png'), fullPage:false})
console.log('saved dev-after-proxy')
await b.close()
