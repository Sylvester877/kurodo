...
// ════════════════════════════════════════════════════════════════════
// AI captions (whisper.cpp) — generate English captions from the episode's
// actual audio when no usable track exists. Offline, MIT, no API key.
let aiSubsApi = null
async function getAiSubsApi() {
  if (!aiSubsApi) aiSubsApi = await import('./ai-subs.js')
  return aiSubsApi
}

app.get('/api/ai-subs/start', async (req, res) => {
  try {
...
app.get('/api/ai-subs/probe', async (req, res) => {
  try {
    const streamUrl = req.query.url ? String(req.query.url) : null
    if (!streamUrl) return badParam(res, 'Missing url')
    ...
    return ok(res, { key, ...st })
  } catch (e) {
    return fail(res, e)
  }
})

app.get('/api/ai-subs/status', async (req, res) => {
  try {
    const key = req.query.key ? String(req.query.key) : ''
    if (!/^[a-f0-9]{24}$/.test(key)) return badParam(res, 'Bad key')
    const { aiSubsStatus, aiSubsVttPath } = await getAiSubsApi()
    const st = aiSubsStatus(key)
    if (st.status === 'unknown' && aiSubsVttPath(key)) {
      // Cached from a previous run (disk artifact exists, job map is empty).
      return ok(res, { status: 'done', pct: 100, url: `/ai-subs/${key}.vtt` })
    }
    return ok(res, st)
  } catch (e) {
    return fail(res, e)
...
app.get('/ai-subs/:key.vtt', (req, res) => {
  const key = String(req.params.key || '')
  if (!/^[a-f0-9]{24}$/.test(key)) return res.status(400).send('Bad key')
  getAiSubsApi().then(({ aiSubsVttPath }) => {
    const p = aiSubsVttPath(key)
    if (!p) return res.status(404).send('Not ready')
