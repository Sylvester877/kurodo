...
app.get('/api/anidap/sources/:slug/:ep/:provider/:type', async (req, res) => {
  ...
  try {
    // 35s hard cap on stream extraction — the SPA player on anidap.lol needs
    // 5-8s to mount + load the m3u8 playlist, and the hidden BrowserWindow
    // navigation adds ~2s. Some providers (gojo, miku) are slower to initialize
    // their embeds, so 20s gives a healthy margin while still failing fast
    // enough that the UI doesn't spin indefinitely. 35s is generous — most
    // providers resolve in 5-15s; 60s was leaving users staring at spinners.
    const data = await Promise.race([
      routedGetStream(anilistId, slug, Number(ep), provider, type, req.query, title),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Stream extraction timed out')), 20_000),
      ),
    ])
    if (!data) {
